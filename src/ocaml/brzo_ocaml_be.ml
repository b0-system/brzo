...
let root_dep dep =
  match String.split_first ~sep:Fpath.natural_dir_sep (Fpath.to_string dep) with
...
let suggest_pkgs_for_modname m ~pkgs =
  ...
  let find_exact m pkgs =
    ...
    | false ->
        match String.split_last ~sep:"_" m with
  ...
  let find_fuzzy m pkgs = (* XXX shouldn't we simply adapt to size ? *)
    let rec loop ~max_dist m pkgs =
      ...
      with
      | [] ->
          begin match String.split_last ~sep:"_" m with
          | Some (m, _) when m <> "" ->
